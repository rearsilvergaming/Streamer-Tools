...
async function processSheet() {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to fetch CSV. Status: ${response.status}, Text: ${response.statusText}, URL: ${csvUrl}`);
        }
        ...
        const parsed = Papa.parse(csvText, {
            ...
            transformHeader: header => {
                ...
                if (trimmedHeader === 'Tags (Optional, comma-separated)' || trimmedHeader === 'Tags (Optional, comma-separated) ') {
        ...
        await fs.writeFile('docs/total-uses.json', JSON.stringify({ total: totalUsesCount }, null, 2));
